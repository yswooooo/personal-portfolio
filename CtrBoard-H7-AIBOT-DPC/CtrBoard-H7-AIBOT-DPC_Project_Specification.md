...
```text
Core/
  main.c, usart.c, gpio.c, dma.c, spi.c, stm32h7xx_it.c
...
- 功能码：0x03。
- 起始寄存器：`LD2_MOTOR_REG_RUN_STATUS` (`PrB.05`)。
- 数量：2，读取 `PrB.05` + `PrB.06`。
...
当前 `Core/Src/main.c` 实际发送 `float vofa_channels[17]`：
...
`vofa_motor_info_t` 结构体包含更多诊断字段，如 speed error、read RTT、write RTT；当前这些字段只在内部维护，没有进入发送数组。
