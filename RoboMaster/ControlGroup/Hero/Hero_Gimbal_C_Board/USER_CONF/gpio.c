...
void GPIO_INIT(void)
{
  ...
  //由原理图可知，引脚拉低，LED亮
	GPIO_ResetBits(GPIOH,GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12);//灭灯
	...
	GPIO_Init(CS1_GYRO_GPIO_Port,&GPIO_InitStructure);
		...
		//轻触开关
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11; //接收高电平引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
...
//	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9; //发送高电平引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_High_Speed;
	GPIO_Init(GPIOE,&GPIO_InitStructure);
		GPIO_SetBits(GPIOE,GPIO_Pin_9);//输出高电平激励信号
