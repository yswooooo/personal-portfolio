...
IMU_Data_t BMI088;
#define GxOFFSET 0.000814495201f
#define GyOFFSET -0.00364227477f
#define GzOFFSET 0.00275207101f
#define gNORM 39.2494049f
...
#define BMI088_CALIBRATE_TIME 10000.0f
void Calibrate_MPU_Offset(IMU_Data_t *bmi088)
{
    static uint8_t buf[8] = {0, 0, 0, 0, 0, 0};
    static int16_t bmi088_raw_temp = 0;
		//static int16_t caliCount = 0;
    ...
    do
    {
        ...
        for (uint16_t i = 0; i <  BMI088_CALIBRATE_TIME; i++)
        {
            ...
            DWT_Delay(0.001);//µ¥Î»ÎªÃë

        }
        for (uint8_t i = 0; i < 3; i++)
            bmi088->GyroOffset[i] = bmi088->Gyro[i] / BMI088_CALIBRATE_TIME;
        bmi088->gNorm = gNorm / BMI088_CALIBRATE_TIME;
...
//        caliCount++;
    } while(0);    //(fabsf(bmi088->GyroOffset[0] - GxOFFSET) / GxOFFSET > 1.0f ||
