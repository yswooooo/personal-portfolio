 ...
 * @brief 串口打包函数
 * 
 * @param cmd_id 命令码，如UI命令码为0x301
 * @param p_data 结构体地址
 * @param len    sizeof结构体长度
 * @param sof    选择发送PC帧头还是裁判系统帧头
 */
      ...
      case STEP_DATA_CRC16:
      {
        ...
        if (p_obj->index >= (HEADER_LEN + CMD_LEN + p_obj->data_len + CRC_LEN))
        {
          ...
          if ( verify_crc16_check_sum(p_obj->protocol_packet, HEADER_LEN + CMD_LEN + p_obj->data_len + CRC_LEN) )
          {
            ...
            {
              judgement_data_handler(p_obj->protocol_packet); //裁判系统接收函数
            }
          }
        }
      }break;
...
uint32_t send_packed_fifo_data(fifo_s_t *pfifo, uint8_t sof)
{
  ...
  if (fifo_count)
  {
    ...
    if (sof == UP_REG_ID){
				...
				APP_FOPS.pIf_DataTx(tx_buf,fifo_count); //USB发送函数 ！！！
			#else
