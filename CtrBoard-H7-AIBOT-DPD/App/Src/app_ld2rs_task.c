/**
  ...
  *
  * @details 每电机状态机 (7 阶段):
  *   IDLE → READ_REQ → WAIT_READ → READ_DONE → WRITE_REQ → WAIT_WRITE → IDLE
  *
...
typedef enum {
    ...
    APP_LD2RS_TASK_PHASE_READ_REQ,           /**< 需要发送读状态请求 (2 寄存器)         */
    APP_LD2RS_TASK_PHASE_WAIT_READ,          /**< 等待读状态应答                        */
    APP_LD2RS_TASK_PHASE_READ_DONE,          /**< 读完成, 处理报警/VOFA/急停            */
    APP_LD2RS_TASK_PHASE_WRITE_REQ,          /**< 需要发送写速度请求                    */
    APP_LD2RS_TASK_PHASE_WAIT_WRITE,         /**< 等待写速度应答                        */
    APP_LD2RS_TASK_PHASE_WRITE_DONE,         /**< 写完成, 本轮结束                      */
} motor_ctrl_phase_t;
...
/**
  ...
  *         本层在读/写阶段各自维护逻辑重试计数 (最多 APP_LD2RS_TASK_MAX_RETRY 次)。
  *
  * @param  fsm       电机运行时句柄
                                                      ...
                                                      bsp_rs485_state_t bus_state)
{
    ...
    case APP_LD2RS_TASK_PHASE_READ_REQ:
        ...
        /* 一次读 2 个连续寄存器: PrB.05 (状态) + PrB.06 (未滤波速度) */
        app_ld2rs_task_build_read_req(fsm->tx_frame, fsm->motor_dev->slave_id,
                          LD2_MOTOR_REG_RUN_STATUS, APP_LD2RS_TASK_MODBUS_READ_QTY_TWO);
        if (bsp_rs485_start_tx(&g_rs485_bus,
            ...
            fsm->tx_start_tick_ms = HAL_GetTick();  /**< 打桩: 读 TX 发起 */
    ...
    case APP_LD2RS_TASK_PHASE_WAIT_READ:
        if (bus_state == BSP_RS485_STATE_DONE) {
            if (app_ld2rs_task_validate_resp(fsm->rx_frame, g_rs485_bus.rx_len,
                ...
                /* 解析 2 个寄存器: [3-4]=PrB.05, [5-6]=PrB.06 */
                fsm->status_word = app_ld2rs_task_get_u16(&fsm->rx_frame[3]);
                fsm->actual_speed_rpm  = app_ld2rs_task_get_u16(&fsm->rx_frame[5]);

                /* 填充 VOFA 反馈转速 + 诊断耗时 */
                fsm->read_rtt_ms = HAL_GetTick() - fsm->tx_start_tick_ms;
                if (fsm->motor_ctrl->motor_id == 1u) {
                    g_vofa_speed.m1_feedback_speed_rpm       = (float)((int16_t)fsm->actual_speed_rpm);
                    g_vofa_speed.m1_read_rtt_ms = (float)fsm->read_rtt_ms;
                } else {
                    g_vofa_speed.m2_feedback_speed_rpm       = (float)((int16_t)fsm->actual_speed_rpm);
                    g_vofa_speed.m2_read_rtt_ms = (float)fsm->read_rtt_ms;
                }

                app_motor_fsm_mark_response_ok(fsm);
                fsm->read_retry_count  = 0u;   /* 成功, 清零逻辑重试计数 */
                fsm->phase = APP_LD2RS_TASK_PHASE_READ_DONE;
            } else {
                /* CRC/站号/功能码校验失败 → 逻辑重试 */
                fsm->read_retry_count++;
                if (fsm->read_retry_count >= APP_LD2RS_TASK_MAX_RETRY) {
                    /* 读重试耗尽 → 强制零速标志 (由 task_run 统一清零 target) */
    ...
    case APP_LD2RS_TASK_PHASE_READ_DONE:
        /* 安全停机 / 报警检查已由 app_ld2rs_task_run 统一处理, 此处仅跳转 */
        fsm->phase = APP_LD2RS_TASK_PHASE_WRITE_REQ;
        break;
