...
/* 急停标志: 0=正常运行, 1=强制停机 (仅 ISR 置位/清零，不操作速度变量) */
volatile uint8_t g_emergency_stop_flag = 1u;
...
void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(TEST_PIN_FOR_SCOPE_GPIO_Port, TEST_PIN_FOR_SCOPE_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : TEST_PIN_FOR_SCOPE_Pin */
  GPIO_InitStruct.Pin = TEST_PIN_FOR_SCOPE_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(TEST_PIN_FOR_SCOPE_GPIO_Port, &GPIO_InitStruct);
